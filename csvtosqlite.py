...
class CsvFileInfo:
    ...
    def save_to_db(self, connection):
        ...
        if self.options.drop_tables:
            try:
                write_out("Dropping " + self.get_table_name())
...
@click.option("--file", "-f",
              ...
              multiple=True)
@click.option("--output", "-o", help="The output database path", default=os.path.basename(os.getcwd()) + ".db")
...
def start(file, output, find_types, drop_tables, verbose):
    ...
    for file in files:
        try:
            ...
            info.process_file()
            write_out("Writing table: " + info.get_table_name())
